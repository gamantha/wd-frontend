...
import IndicatorForm from '../../views/Indicator/IndicatorForm'
...
import {
  ...
  updateIndicator,
  deleteIndicator,
} from '../../store/indicator/indicator.actions'
...
class Indicator extends Component {
  ...
  componentDidMount() {
    this.props
      .fetchIndicators({ page: 1, limit: 10 })
      .then(res => {
        console.log(res, 'indicator fetched')
      })
      .catch(err => {
  ...
  onChangePage = (page, pageSize) => {
    this.props.refetch({ page, limit: pageSize }).then(res => {
      console.log(res, 'indicator: change page')
    })
  }
  ...
  render() {
    const { loading, error, refetch, indicators = [] } = this.props
...
const mapDispatchToProps = dispatch => {
  return {
    ...bindActionCreators(
      {
        fetchIndicators,
        onCreateItem: addIndicator,
        refetch: fetchIndicators,
        onUpdateItem: updateIndicator,
        onDeleteItem: deleteIndicator,
      },
      dispatch
    ),
  }
